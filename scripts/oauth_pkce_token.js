...
async function dynamicRegisterClient(registerUrl) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Client registration failed: ${res.status} ${res.statusText}: ${t}`);
  }
  return res.json();
}
...
async function main() {
  ...
  const authorizationUrl = new URL(meta.authorization_endpoint || '/oauth2/authorize', issuer).toString();
  const tokenUrl = new URL(meta.token_endpoint || '/oauth2/token', issuer).toString();
  const registerUrl = meta.registration_endpoint ? new URL(meta.registration_endpoint, issuer).toString() : null;

  let clientId = process.env.OAUTH_CLIENT_ID;
  if (!clientId) {
    if (!registerUrl) {
      throw new Error('OAUTH_CLIENT_ID not set and registration_endpoint not available. Provide a client ID.');
    }
    console.log('Registering a dynamic client...');
    const registration = await dynamicRegisterClient(registerUrl);
    clientId = registration.client_id;
    if (!clientId) {
      throw new Error(`Dynamic registration succeeded but no client_id returned: ${JSON.stringify(registration)}`);
    }
    console.log(`Registered client_id: ${clientId}`);
  }
  if (!clientId) {
    throw new Error('client_id is undefined; cannot proceed with authorization.');
  }
    ...
    try {
      if (req.url && req.url.startsWith('/callback')) {
        ...
        if (!tokenRes.ok) {
          ...
          throw new Error(`Token exchange failed: ${tokenRes.status} ${tokenRes.statusText}: ${t}`);
        }
        ...
        await fs.promises.writeFile(tokenPath, JSON.stringify(payload, null, 2), 'utf8');

        res.statusCode = 200;
        res.setHeader('content-type', 'text/html');
        res.end('<h1>Login complete</h1><p>You may close this window.</p>');

        console.log(`Saved token to ${path.relative(process.cwd(), tokenPath)}`);
  ...
  srv.listen(redirectPort, redirectHost, () => {
    ...
    const actualPort = (addr && typeof addr !== 'string') ? addr.port : redirectPort;
    ...
    if (!hasClientId) {
      console.error('ERROR: client_id not found in constructed URL. URL construction failed.');
    ...
    const opener = isMac ? 'open' : (process.platform === 'win32' ? 'start' : 'xdg-open');
    ...
    const timeoutHandle = setTimeout(() => {
      if (!browserOpened) {
        console.log('Browser did not open. Copy and paste the URL above into your browser.');
      }
    }, 2000);
