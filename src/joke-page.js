...
function handleFeedback(req) {
  try {
    if (req.method === 'POST') {
      ...
      const feedbackData = JSON.parse(sharedStorage.getItem(jokeId) || '{"thumbsUp":0,"thumbsDown":0}');
      ...
      console.log('Feedback recorded for joke ' + jokeId + ' - Up: ' + feedbackData.thumbsUp + ', Down: ' + feedbackData.thumbsDown);
      ...
      return {
        status: 400,
        body: JSON.stringify({ error: 'GET not supported for feedback endpoint' }),
        contentType: 'application/json'
      };
    }
